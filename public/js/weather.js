...
async function getWeather(city = 'Warszawa') { 
    try {
        ...
        document.querySelector('.date-dayname').textContent = todayDayName;

        const date = new Date().toUTCString();
        const extractedDateTime = date.slice(5, 16);
        document.querySelector('.date-day').textContent = extractedDateTime;

        const currentWeatherIconCode = dailyForecast[todayDayName].icon;
        ...
        document.querySelector('.weather-desc').textContent = capitalizeFirstLetters(dailyForecast[todayDayName].description);

        document.querySelector('.humidity .value').textContent = dailyForecast[todayDayName].humidity + ' %';
        document.querySelector('.wind .value').textContent = dailyForecast[todayDayName].windSpeed + ' m/s';
...
document.addEventListener("DOMContentLoaded", function () {
    const initialCity = document.getElementById('city').value || 'Warszawa';  // Domyślne miasto
    getWeather(initialCity);
    setInterval(() => getWeather(initialCity), 900000);  // Odświeżanie co 15 minut
});
